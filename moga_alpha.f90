program moga_alpha
  ...
  use dynap_mod
  use crm_mod
  use alpha_modules !high-order momentum compaction calculation up to the third order
  ...
  use namelist_moga_alpha ! moga_output_file, set_chrom_x, set_chrom_y, initial_pop, seed, breeder_params, max_gen, alpha1max, co_limit,
  ...
  real(rp) delta_e
  real(rp) alpha_0, alpha_1, alpha_2, alpha1_0
  logical linear_ok
  logical feasible
  ...
  !alpha variables call getalpha(ring, N, n_slices, nux, alpha0, alpha1, alpha2)
  integer N, n_slices
  real(rp) nux
  real(rp) alpha0, alpha1, alpha2
...
n_slices = 50
 ...
 !Initialisation alpha1_0
!if (ring%param%geometry == closed$) call twiss_at_start(ring)
!call twiss_propagate_all(ring)
nux = ring%ele(ring%n_ele_track)%a%phi/2./pi
call getalpha(ring, N, n_slices, nux, alpha0, alpha1_0, alpha2)
print *, alpha1_0

if (alpha1max .lt. 0) then
                minimisation = .true.
else
                minimisation = .false.
endif
  ...
  allocate(objs(dim))! Ajout une dimension a objs pour ajouter alpha
  ...
  if( master ) then
   ...
   allocate(sel(lambda))
    ...
    write(21,'(a6,50a19)') '# id', (trim(l_mags(i)%name)//'['//trim(l_mags(i)%property)//']',i=1,n_linear), (trim(c_mags(i)%name),i=1,n_chrom), (trim(h_mags(i)%name),i=1,n_harmo), "o1", "o2", "o3", "o4", "feasible"
    close(21)
    open(unit=21, iostat=iostat, file=thin_file, access='append')
    write(21,'(a6,50a10)') '# id', (trim(l_mags(i)%name)//'['//trim(l_mags(i)%property)//']',i=1,n_linear), (trim(c_mags(i)%name),i=1,n_chrom), (trim(h_mags(i)%name),i=1,n_harmo), "o1", "o2", "o3", "o4", "feasible"
    close(21)
    ...
    do while(.true.)
	  ...
	  if (.not. minimisation) then
                               ...
                               call twiss_and_track(ring,co,status)
                               call getalpha(ring, N, n_slices, nux, alpha0, alpha1, alpha2)
                               cons(8) = min(1-alpha1/alpha1max, cons(8))
      endif
      ...
      if( feasible .and. (generate_feasible_seeds_only .lt. 0) ) then
		   ...
		   call getalpha(ring, N, n_slices, nux, alpha0, alpha1, alpha2)
                      if (minimisation) then
                         objs(4) = abs(alpha1/alpha1_0)
                      else
                           if (alpha1 .lt. alpha1max) then
                              objs(4) = abs(alpha1/alpha1max)
                           else
                              objs(4) = 1.0d0
                           endif
                      endif
